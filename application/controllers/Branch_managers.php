...
class Branch_managers extends Persons
{
	public function __construct()
	{
		parent::__construct('branch_managers');
	}
	
	/*
	Returns BranchManager table data rows. This will be called with AJAX.
	*/
	public function search()
	{
		...
		$employees = $this->BranchManager->search($search, $limit, $offset, $sort, $order);
		$total_rows = $this->BranchManager->get_found_rows($search);
	...
	*/
	...
	{
		$suggestions = $this->xss_clean($this->BranchManager->get_search_suggestions($this->input->post('term')));
	...
	*/
	...
	{
		$person_info = $this->BranchManager->get_info($employee_id);
	...
	*/
	public function save($employee_id = -1)
	{
		...
		if($this->Employee->saveBanchManager($person_data, $employee_data, $grants_array, $employee_id))
		{
			// New employee
			if($employee_id == -1)
			{
